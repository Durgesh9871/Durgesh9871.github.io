import React from 'react'
import { Box , Text} from '@chakra-ui/react'
import '../CssPages/Skills.css';
import AOS from 'aos';
import 'aos/dist/aos.css'

import Calender from './Calender';
import Stats from './Stats';
const Skills = () => {
  AOS.init();
  
  return (
    <div id="SkillScroll">
        <div  className='skill' >
           <h1 style={{marginTop:"270px"}}>Skills</h1>
           <div className='SkillGrid'  >
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center"   boxShadow= "4px 4px #C05621, -0em 1px .4em #C05621" >
                  <img src="https://chiranjeev-thapliyal.vercel.app/svg/html-5.svg" />
                  <h1 style={{color:"#C05621"}}>HTML</h1>
                 </Box>
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #4299E1, -0em 1px .4em #4299E1" >
                 <img src="https://chiranjeev-thapliyal.vercel.app/svg/css3.svg" />
                  <h1 style={{color:"#4299E1"}}>CSS</h1>
                 </Box>
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #F6E05E, -0em 1px .4em #F6E05E">
                 <img src="https://chiranjeev-thapliyal.vercel.app/svg/javascript.svg" />
                  <h1 style={{color:"#F6E05E"}}>Javascript</h1>
                 </Box>


                 <Box data-aos="fade-down"
    data-aos-offset="-300"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #6B46C1, -0em 1px .4em #6B46C1"  >
                 <img src="https://chiranjeev-thapliyal.vercel.app/svg/redux.svg" />
                  <h1 style={{color:"#6B46C1"}}>REDUX</h1>
                 </Box>

                 <Box  data-aos="fade-down"
    data-aos-offset="-300"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #0BC5EA, -0em 1px .4em #0BC5EA">
                 <img src="https://chiranjeev-thapliyal.vercel.app/svg/reactjs.svg" />
                  <h1 style={{color:"#0BC5EA"}}>REACT</h1>
                 </Box>
                
                 <Box data-aos="fade-down"
    data-aos-offset="-300"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center"
                  boxShadow= "4px 4px #4299E1, -0em 1px .4em #4299E1" >
                 <img style={{borderRadius:"50%"}} src="	https://velvety-croquembouche-9aebc1.netlify.app/static/media/typescript.f66836b8.svg" />
                  <h1 style={{color:"#4299E1"}}>Typescript</h1>
                 </Box>
                 <Box  data-aos="fade-down"
    data-aos-offset="-300"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #76E4F7, -0em 1px .4em #76E4F7">
                 <img style={{borderRadius:"50%"}} src="https://pbs.twimg.com/profile_images/1244925541448286208/rzylUjaf_400x400.jpg" />
                  <h1 style={{color:"#76E4F7"}}>Chakra UI</h1>
                 </Box>
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #A0AEC0, -0em 1px .4em #A0AEC0" >
                 <img src="https://chiranjeev-thapliyal.vercel.app/svg/nodejs.svg" />
                  <h1 style={{color:"#A0AEC0"}}>Node JS</h1>
                 </Box>
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #48BB78, -0em 1px .4em #48BB78" ><img src="https://chiranjeev-thapliyal.vercel.app/svg/mongodb.svg" />
                  <h1 style={{color:"#48BB78"}}>MongoDB</h1></Box>
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #718096, -0em 1px .4em #718096" >
                 <img style={{borderRadius:"50%"}}  src="data:image/png;base64," />
                  <h1 style={{color:"#718096"}}>ExpressJS</h1>
                 </Box>

 
             </div>
{/*  Tools are start --------------------- */}
                 <Text style={{marginTop:"100px" , color:"salmon" , fontSize:"30px" , fontWeight:"700" ,border:"2px  red" ,display:"flex" ,justifyContent:"center"}}>Tools</Text>
                 {/* Tool code -- */}
                 <div className='SkillGrid'>
                  
                  {/* Github */}
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #EBF8FF, -0em 1px .4em #EBF8FF"  >
                 <img style={{borderRadius:"50%"}} src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png" />
                  <h1 style={{color:"#EBF8FF"}}>Github</h1>
                 </Box>
                 {/* Netlify */}
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #EBF8FF, -0em 1px .4em #EBF8FF"  >
                 <img style={{borderRadius:"50%"}} src="https://img.icons8.com/external-tal-revivo-shadow-tal-revivo/512/external-netlify-a-cloud-computing-company-that-offers-hosting-and-serverless-backend-services-for-static-websites-logo-shadow-tal-revivo.png" />
                  <h1 style={{color:"#EBF8FF"}}>Netlify</h1>
                 </Box>
                 {/* Postman */}
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #EBF8FF, -0em 1px .4em #EBF8FF"  >
                 <img style={{borderRadius:"50%"}} src="https://img.icons8.com/external-tal-revivo-color-tal-revivo/512/external-postman-is-the-only-complete-api-development-environment-logo-color-tal-revivo.png" />
                  <h1 style={{color:"#EBF8FF"}}>Postman</h1>
                 </Box>

                 {/* Vs code */}
                 <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #EBF8FF, -0em 1px .4em #EBF8FF"  >
                 <img style={{borderRadius:"50%"}} src="https://img.icons8.com/color/512/visual-studio-code-2019.png" />
                  <h1 style={{color:"#EBF8FF"}}>Vs Code</h1>
                 </Box>
                  
                  {/* Vercel---- */}
                  <Box data-aos="fade-down"
    data-aos-offset="-100"
    data-aos-delay="50"
    data-aos-duration="1000"
    data-aos-easing="ease-in-out"
    data-aos-mirror="true"
    data-aos-once="false"
    data-aos-anchor-placement="top-center" boxShadow= "4px 4px #EBF8FF, -0em 1px .4em #EBF8FF"  >
                 <img style={{borderRadius:"50%"}} src="https://img.icons8.com/material-outlined/512/verge.png" />
                  <h1 style={{color:"#EBF8FF"}}>Vercel</h1>
                 </Box>

                 
                 </div>


        </div>
       
        
        <Calender />
        
       
          <Stats />
        
       
        
    </div>
  )
}

export default Skills